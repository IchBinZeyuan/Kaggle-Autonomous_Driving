...
class Routine(object):
    ...
    def run(self):
        ...
        df_train, df_dev = train_test_split(self.train_data, test_size=0.01, random_state=42)
        df_test = self.test_data

        # Create dataset objects
        train_dataset = DataLoading(self._settings, df_train, train_images_dir)
        dev_dataset = DataLoading(self._settings, df_dev, train_images_dir)
        test_dataset = DataLoading(self._settings, df_test, test_images_dir)
        ...
        self.show_result(model, dev_dataset, df_dev)

    def show_result(self, model, dev_dataset, df_dev):
        train_images_dir = self.path + 'train_images/{}.jpg'
        for idx in range(8):
            img, mask, regr = dev_dataset[idx]

            output = model(torch.tensor(img[None]).to(self.device)).data.cpu().numpy()
            ...
            axes[0].imshow(dev_dataset.visualize(img, coords_true))
            axes[1].set_title('Prediction')
            axes[1].imshow(dev_dataset.visualize(img, coords_pred))
            plt.show()
